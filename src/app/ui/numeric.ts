...
/**
 * Чувствительность мелкого режима на дорожке ползунка. Меньше, чем у поля с числом: дорожка
 * задаёт значение положением, и слишком мелкий шаг сделал бы её бесполезной.
 */
const TRACK_FINE_SENSITIVITY = 0.15;

/** Значение по абсолютному положению указателя на дорожке: щелчок сразу ставит бегунок. */
export function valueFromTrackPosition(
  ...
  if (trackWidth <= 0) return clamp(range.min, range.min, range.max);
  const fraction = clamp((pointerX - trackLeft) / trackWidth, 0, 1);
  return snapToStep(range.min + fraction * (range.max - range.min), range, mode);
}

/**
 * Значение по сдвигу вдоль дорожки от точки нажатия. Относительный отсчёт нужен, чтобы
 * зажатый Shift замедлял движение: при абсолютном отсчёте бегунок просто прыгал бы под курсор.
 */
export function valueFromTrackDrag(
  startValue: number,
  dx: number,
  trackWidth: number,
  range: NumericRange,
  ...
  if (trackWidth <= 0) return snapToStep(startValue, range, mode);
  const sensitivity = mode === 'fine' ? TRACK_FINE_SENSITIVITY : 1;
  const delta = (dx / trackWidth) * (range.max - range.min) * sensitivity;
  return snapToStep(startValue + delta, range, mode);
}

/** Доля заполнения дорожки, 0..1. Нужна только для показа. */
export function trackFraction(value: number, range: NumericRange): number {
